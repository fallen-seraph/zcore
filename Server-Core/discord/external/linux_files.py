...
class linux_files:
    def __init__(self):
        ...
        self.__systemctlPath = PurePosixPath(self.__home).joinpath(".config/systemd/user")
        ...
        self.__save = PurePosixPath(self.__zomboid).joinpath("Saves/Multiplayer/pzserver")
        self.__zomboidBackups = PurePosixPath(self.__zomboid).joinpath("backups")

        self.__serverini = PurePosixPath(self.__zomboid).joinpath("Server/pzserver.ini")
        self.__accountDB = PurePosixPath(self.__zomboid).joinpath("db/pzserver.db")
    ...
    def create_linuxgsm(self):
        ...
        try:
            ...
            try:
                result = run(["bash", "linuxgsm.sh", "pzserver"], cwd=self.__pzlgsm, check=True, text=True, shell=False, capture_output=True)
                print(result)
